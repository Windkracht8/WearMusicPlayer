...
import com.windkracht8.wearmusicplayer.data.Library.Track
import com.windkracht8.wearmusicplayer.logE
import com.windkracht8.wearmusicplayer.toastFromBG
...
object Playlists {
	...
	private fun create(json: String): Playlist? {
		try { return create(JSONObject(json)) }
		catch(e: Exception) {
			logE("Playlists.create : ${e.message}")
		}
		return null
	}
	fun create(obj: JSONObject): Playlist? {
		try {
			...
			for(index in 0 until tracks.length()) {
				...
				Library.tracks.find { it.path == tracks.getString(index) }?.let {
